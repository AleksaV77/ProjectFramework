...
class CourseTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.course = Course.objects.create(
            course_name="Django",
            course_description="как использовать ViewSet",
            owner=self.user,
        )
        self.lesson = Lesson.objects.create(
            lesson_name="Вьюсеты", course=self.course, owner=self.user
        )
    ...
    def test_course_update(self):
        ...
        data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(data.get("course_name"), "Generic2")
...
class LessonTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.lesson = Lesson.objects.create(
            ...
            course=self.course,
            owner=self.user,
            lesson_description="Знакомство с ViewSet",
        )
        self.client.force_authenticate(user=self.user)
